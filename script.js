...
function nowyProdukt() {
    let prod = new Produkt(
        formularz.nazwa.value,
        formularz.ilosc.value,
        formularz.cena.value
    );
...
function dodajDoTabeli(id, produkt) {
    ...
    edytuj.innerHTML =
        "<span class='material-icons' id='icon-edit' onclick='edytujProdukt(this)'>edit</span>";
    usun.innerHTML =
        "<span class='material-icons' id='icon-delete' onclick='usunProdukt(this)'>delete</span>";
}
...
function edytujProdukt(prod) {
    ...
    formularzEdycji.onsubmit = (event) => {
        ...
        produktEdytowany.suma =
            formularzEdycji.ilosc.value * formularzEdycji.cena.value;
...
function sortuj(n) {
    ...
    if (sortujRosnaco[n] && n != 0) listaProduktow.reverse();
